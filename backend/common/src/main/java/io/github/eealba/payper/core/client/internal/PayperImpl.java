...
class PayperImpl extends Payper {
    ...
    class PayerResponseSpecImpl<R1, R2> implements PayperResponse.PayperResponseSpec<R1, R2> {
            ...
            private void call(boolean discard) {
                if (statusCode == 0) {
                    if (discard) {
                        ...
                        Response<byte[]> res = webClient.send(Mapper.mapRequest(config, request, getToken().join()),
        ...
        private CompletableFuture<TokenImpl> createToken() {
            PayperAuthenticator authenticator = config.authenticator();

            URL url;
            try {
                url = URI.create(authenticator.getBaseUrl() + "/v1/oauth2/token").toURL();
