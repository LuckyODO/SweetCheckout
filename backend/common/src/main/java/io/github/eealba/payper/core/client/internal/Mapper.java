...
class Mapper {
    ...
    static Request mapRequest(PayperConfig config, PayperRequest request, TokenImpl token) {
        ...
        try {
            builder.uri(URI.create(config.authenticator().getBaseUrl() + request.path()).toURL());
        ...
        switch (request.method()) {
            case GET:
                ...
                break;
            case POST:
                builder.POST(mapPublisher(request.bodyPublisher().orElse(null)));
                break;
            case PUT:
                builder.PUT(mapPublisher(request.bodyPublisher().orElse(null)));
                break;
            case DELETE:
                builder.DELETE();
                break;
            case PATCH:
                builder.PATCH(mapPublisher(request.bodyPublisher().orElse(null)));
                break;
        }
        return builder.build();
    }
