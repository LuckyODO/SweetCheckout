    ...
    class PayperAuthenticators {
        ...
        private static Supplier<char[]> systemSupplier(String key) {
            return () -> {
                ...
                Optional<String> opt;
                if (value == null) {
                    opt = Optional.ofNullable(System.getenv(key));
                } else {
                    opt = Optional.of(value);
                }
                return opt.orElseThrow(() -> new PayperException("Not found environment variable or System property: " + key)).toCharArray();
            };
        }
        ...
        public static PayperAuthenticator ofSandBox(InputStream inputStream) throws IOException {
            ...
            char[] clientId = props.getProperty("PAYPAL-CLIENT-ID").toCharArray();
            char[] clientSecret = props.getProperty("PAYPAL-CLIENT-SECRET").toCharArray();
            return of(() -> API_SANDBOX_PAYPAL_COM, () -> clientId, () -> clientSecret);
        }
